...
class GenerateKexts:
    ...
    def _is_build_beta(self, kext: str, version: str) -> bool:
        # Load CHANGELOG.md
        ...
        # Check if kext is 'rolling' or 'beta'
        for line in changelog.split("\n"):
            if kext in line and version in line:
                if ("rolling" in line or "beta" in line):
                    return True
                break
        ...
        for variant in ["RELEASE", "DEBUG"]:
            ...
            if packaging.version.parse(remote_version) <= packaging.version.parse(local_version):
                ...
                if remote_version == local_version:
                    if self._is_build_beta(kext_name, local_version) is False:
                        continue
                    print(f"  {kext_name} {variant} is a beta build, updating...")
                else:
                    continue
    ...
    def _update_constants_file(self, variable_name, old_version, new_version):
        ...
        constants_file = Path("../../oclp_elite/constants.py")
