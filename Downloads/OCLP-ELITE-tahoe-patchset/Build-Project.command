#!/usr/bin/env python3
"""
Build-Project.command: Generate OCLP-ELITE.app and OCLP-ELITE.pkg
"""
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(description="Build OCLP-ELITE Suite", add_help=False)
    ...
    if (args.run_as_individual_steps is False) or (args.run_as_individual_steps and args.prepare_package):
        # Build OCLP-ELITE.pkg and OCLP-ELITE-Uninstaller.pkg
        package.GeneratePackage().generate()

        # Sign OCLP-ELITE.pkg
        sign_notarize.SignAndNotarize(
            path=Path("dist/OCLP-ELITE.pkg"),
        ...
        # Sign OCLP-ELITE-Uninstaller.pkg
        sign_notarize.SignAndNotarize(
            path=Path("dist/OCLP-ELITE-Uninstaller.pkg"),
            signing_identity=args.installer_signing_identity,
            notarization_apple_id=args.notarization_apple_id,
            notarization_password=args.notarization_password,
            notarization_team_id=args.notarization_team_id,
