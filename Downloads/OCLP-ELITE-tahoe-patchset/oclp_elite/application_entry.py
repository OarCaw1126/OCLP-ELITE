...
class OpenCoreLegacyPatcher:
    """
    Initial entry point for starting OCLP-ELITE
    """
    ...
    def _generate_base_data(self) -> None:
        ...
        if "python" in launcher_binary:
            ...
            if "main.py" in launcher_script:
                launcher_script = launcher_script.replace("/resources/main.py", "/OCLP-ELITE-GUI.command")
        ...
        if self.constants.commit_info[0] not in ["Running from source", "Built from source"]:
            # Now that we have commit info, update beta link
            branch = self.constants.commit_info[0]
            branch = branch.replace("refs/heads/", "")
            self.constants.installer_pkg_url_beta = self.constants.installer_pkg_url_beta.replace("main", branch)
