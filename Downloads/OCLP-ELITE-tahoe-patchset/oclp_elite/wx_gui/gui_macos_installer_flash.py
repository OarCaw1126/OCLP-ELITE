...
class macOSInstallerFlashFrame(wx.Frame):
    ...
    def _auto_package_handler(self):
        ...
        Note the following:
            ...
            - When running from source/unable to find on Github, use the beta.link variant
            - If beta also fails, fall back to the manually uploaded variant
        """
        link = self.constants.installer_pkg_url
        if network_handler.NetworkUtilities(link).validate_link() is False:
            logging.info("Stock Install.pkg is missing on Github, falling back to Beta")
            link = self.constants.installer_pkg_url_beta
