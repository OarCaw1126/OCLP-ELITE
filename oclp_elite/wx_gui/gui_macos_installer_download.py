...
class macOSInstallerDownloadFrame(wx.Frame):
    ...
    def _generate_elements(self, frame: wx.Frame = None) -> None:
        ...
        # Button: Download macOS Installer
        download_button = wx.Button(frame, label="Download macOS Installer（.app）", pos=(-1, title_label.GetPosition()[1] + title_label.GetSize()[1] + 5), size=(200, 30))
        download_button.Bind(wx.EVT_BUTTON, self.on_download)
        download_button.Centre(wx.HORIZONTAL)

        # Button: Use existing macOS Installer
        existing_button = wx.Button(frame, label="Use existing macOS Installer（.app）", pos=(-1, download_button.GetPosition()[1] + download_button.GetSize()[1] - 5), size=(200, 30))
        existing_button.Bind(wx.EVT_BUTTON, self.on_existing)
        existing_button.Centre(wx.HORIZONTAL)

        shdmg_button = wx.Button(frame, label="Download macOS Installer（.dmg）", pos=(-1, existing_button.GetPosition()[1] + existing_button.GetSize()[1] - 5), size=(200, 30))
        shdmg_button.Bind(wx.EVT_BUTTON, self.on_downdmg)
        shdmg_button.Centre(wx.HORIZONTAL)

        fldmg_button = wx.Button(frame, label="Burn Image（.dmg）", pos=(-1, shdmg_button.GetPosition()[1] + shdmg_button.GetSize()[1] - 5), size=(200, 30))
        fldmg_button.Bind(wx.EVT_BUTTON, self.on_flashdmg)
        fldmg_button.Centre(wx.HORIZONTAL)

        # Button: Return to Main Menu
        return_button = wx.Button(frame, label="Return to Main Menu", pos=(-1, fldmg_button.GetPosition()[1] + fldmg_button.GetSize()[1] + 5), size=(150, 30))
    ...
    def _generate_catalog_frame(self) -> None:
        ...
        title_label = wx.StaticText(self, label="Searching for downloadable versions", pos=(-1,5))
        title_label.SetFont(gui_support.font_factory(19, wx.FONTWEIGHT_BOLD))
        title_label.Centre(wx.HORIZONTAL)
    ...
    def _generate_dmg_frame(self) -> None:
        ...
        # 标题: 正在查找可下载的版本
        title_label = wx.StaticText(self, label="Searching for downloadable DMG images", pos=(-1, 5))
        title_label.SetFont(gui_support.font_factory(19, wx.FONTWEIGHT_BOLD))
        title_label.Centre(wx.HORIZONTAL)
        ...
        def _fetch_dmg():
            ...
            try:
                ...
                if response.status_code == 200:
                    ...
                    logging.info("Returned JSON data:")
    ...
    def _display_available_dmgs(self, event: wx.Event = None, show_full: bool = False) -> None:
        """
        Display available DMGs in the frame
        """
        ...
        self.frame_modal = wx.Dialog(self, title="Select SimpleHac DMG image", size=(dialog_width, dialog_height))

        title_label = wx.StaticText(self.frame_modal, label="Select this mirror powered by SimpleHac", pos=(-1, -1))
        title_label.SetFont(gui_support.font_factory(19, wx.FONTWEIGHT_BOLD))
        ...
        # 创建按钮，父窗口设为rectbox
        self.copy_button = wx.Button(rectbox, label="Copy link", size=(70, 30))
        self.select_button = wx.Button(rectbox, label="Download", size=(130, 30))
        ...
        self.select_button.SetToolTip("Download the selected DMG")
        self.select_button.SetDefault()
        self.copy_button.SetToolTip("Copy the download link of the selected DMG file")
        ...
        self.olderversions_checkbox = wx.CheckBox(self.frame_modal, label="Show older/test versions")
        ...
        return_button = wx.Button(self.frame_modal, label="Return To Main Menu", size=(150, 30))
    ...
    def _display_available_installers(self, event: wx.Event = None, show_full: bool = False) -> None:
        ...
        dialog_width = 505 if show_full else 500
        dialog_height = 500 if show_full else 380
        self.frame_modal = wx.Dialog(self, title="Select macOS Installer", size=(dialog_width, dialog_height))
    
        # Title: Select macOS Installer
        title_label = wx.StaticText(self.frame_modal, label="Select this macOS", pos=(-1,-1))
        title_label.SetFont(gui_support.font_factory(19, wx.FONTWEIGHT_BOLD))
        ...
        if installers:
            ...
            wx.MessageDialog(self.frame_modal, "Unable to retrieve update information from Apple's servers。", "Error", wx.OK | wx.ICON_ERROR).ShowModal()
        ...
        self.copy_button = wx.Button(rectbox, label="Copy link", size=(70, 30))
        self.select_button = wx.Button(rectbox, label="Select", size=(130, 30))
        ...
        self.select_button.SetToolTip("Download the selected macOS")
        self.select_button.SetDefault()
        ...
        return_button = wx.Button(self.frame_modal, label="Return", pos=(-1, -1), size=(150, -1))
        return_button.Bind(wx.EVT_BUTTON, self.on_return_to_main_menu)
        return_button.SetFont(gui_support.font_factory(13, wx.FONTWEIGHT_NORMAL))
    ...
    def on_copy_link(self, installers: dict) -> None:
        ...
        if selected_item != -1:
            ...
            wx.MessageDialog(self.frame_modal, "Copy to clipboard", "", wx.OK | wx.ICON_INFORMATION).ShowModal()
    ...
    def on_download_dmg(self, installers: dict) -> None:
        ...
        if selected_item != -1:
            ...
            dir_dialog = wx.DirDialog(self, "Select save directory", "", wx.DD_DIR_MUST_EXIST)
            ...
            gui_download.DownloadFrame(
                self,
                title="Download image from SimpleHac OSS",
